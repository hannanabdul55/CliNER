...
def lists_to_dataset_structure(sentences_tokens, sentence_tags, total_token_counter, token_count,
...
class Dataset(object):
    ...
    def _parse_dataset(self, dataset_filepath, dataset_type, sentences_list=[], tags_list=[],
                       Not_here=False):
        ...
        if Not_here == False:
            labels, tokens, token_count, label_count, character_count, features, feature_file_name, feature_vector_size = lists_to_dataset_structure(
                sentences_list, tags_list, total_token_counter, token_count, label_count,
                character_count)
    ...
    def _convert_to_indices(self, dataset_types):
        ...
        for dataset_type in dataset_types:
            ...
            for token_sequence in tokens[dataset_type]:
                ...
                character_indices_padded[dataset_type].append([hd.pad_list(temp_token_indices,
                                                                           longest_token_length_in_sequence,
                                                                           self.PADDING_CHARACTER_INDEX)
                                                               for temp_token_indices in
                                                               character_indices[dataset_type][
                                                                   -1]])
    ...
    def update_dataset(self, dataset_filepaths, dataset_types, Datasets_tokens, Datasets_labels):
        ...
        for dataset_type in dataset_types:
            ...
            self.labels[dataset_type], self.tokens[
                dataset_type], _, _, _, _, _, _ = self._parse_dataset("", dataset_type,
                                                                      Datasets_tokens[
                                                                          dataset_type],
                                                                      Datasets_labels[
                                                                          dataset_type])
    ...
    def load_dataset(self, avaliable_datasets_sent, avaliable_datasets_labels, dataset_filepaths,
                     parameters, token_to_vector=None, pretrained_dataset=None):
        '''
        dataset_filepaths : dictionary with keys 'train', 'valid', 'test', 'deploy'
        '''
        ...
        for dataset_type in ['train', 'valid', 'test', 'deploy']:
            ...
            if Not_here == False:
                labels[dataset_type], tokens[dataset_type], token_count[dataset_type], label_count[
                    dataset_type], character_count[dataset_type], features[dataset_type], \
                features_file_names[dataset_type], feature_vector_size[dataset_type] \
                    = self._parse_dataset("", dataset_type,
                                          sentences_list=avaliable_datasets_sent[dataset_type],
                                          tags_list=avaliable_datasets_labels[dataset_type])
        ...
        for character in list(character_count['train'].keys()) + list(
                character_count['valid'].keys()) + list(character_count['test'].keys()) + list(
        ...
        for character in list(label_count['train'].keys()) + list(
            ...
            label_count['all'][character] = label_count['train'][character] + label_count['valid'][
                character] + label_count['test'][character] + label_count['deploy'][character]
        ...
        if self.verbose: print(
            "len(token_count['train'].keys()): {0}".format(len(token_count['train'].keys())))
        for token, count in token_count['all'].items():
            ...
            if parameters['remap_unknown_tokens_to_unk'] == 1 and \
                    ...
                    not hd.is_token_in_pretrained_embeddings(token, token_to_vector,
                                                             parameters) and \
            ...
            for label in label_count['all']:
                if label not in pretraining_dataset.label_to_index:
                    raise AssertionError(
                        "The label {0} does not exist in the pretraining dataset. ".format(label) +
