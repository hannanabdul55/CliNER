...
class ClinerModel:
    ...
    def train(self, train_notes, val=[], test=[]):
        ...
        if val:
            ...
            self.train_fit(train_sents, train_labels, val_sents=val_sents, val_labels=val_labels,
...
def generic_train(p_or_n, train_sents, train_labels, use_lstm, val_sents=None, val_labels=None,
                  test_sents=None, test_labels=None, dev_split=None):
    '''
    generic_train()
    ...
    if use_lstm:
        ...
        pickle.dump(dataset,
                    open(os.path.join(parameters['model_folder'], 'dataset.pickle'), 'wb'))
        ...
        while epoch_number < 90:
            ...
            if epoch_number != 0:
                ...
                for sequence_number in sequence_numbers:
                    ...
                    if step % 10 == 0:
                        print(
                            'Training {0:.2f}% done\r'.format(step / len(sequence_numbers) * 100), end='\r')

                model_saver.save(sess, os.path.join(parameters['model_folder'],
            ...
            if epoch_number > 0:
                f1, predictions = training_predict_LSTM.prediction_step(sess, dataset, "test",
                                                                        model, epoch_number,
                                                                        parameters[
                                                                            'conll_like_result_folder'],
                                                                        transition_params_trained)
                f1_train, _ = training_predict_LSTM.prediction_step(sess, dataset, "train", model,
                                                                    epoch_number, parameters[
                                                                        'conll_like_result_folder'],
                                                                    transition_params_trained)
                f1_valid, _ = training_predict_LSTM.prediction_step(sess, dataset, "valid", model,
                                                                    epoch_number, parameters[
                                                                        'conll_like_result_folder'],
                                                                    transition_params_trained)

                correctly_predicted_tokens = training_predict_LSTM.compute_train_accuracy(
                    parameters['conll_like_result_folder'] + "valid" + os.sep + "epoche_" + str(
                        epoch_number) + ".txt")
...
def generic_predict(p_or_n, tokenized_sents, vocab, clf, use_lstm, hyperparams):
    ...
    if use_lstm:
        ...
        with sess.as_default():
            ...
            # model=entity_model.EntityLSTM(dataset,parameters)
            transition_params_trained = model.restore_from_pretrained_model(parameters, dataset,
                                                                            sess,
